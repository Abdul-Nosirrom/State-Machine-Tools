...
public class InputBuffer
{
    ...
    public void Update()
    {
        //DataManager.dataManager.playerInputBuffer = this; CHECK ON THIS IF ERROR
        if (buffer.Count < InputManager.Instance.inputData.rawInputs.Count || buffer.Count == 0)
        {
            InitializeBuffer();
        }
        ...
        for (int r = 0; r < buttonCommandCheck.Count; r++)
        {
            ...
            if (InputManager.Instance.inputData.rawInputs[r].inputType == RawInput.InputType.IGNORE) { buttonCommandCheck[r] = 0; }
        ...
        for (int m = 0; m < motionCommandCheck.Count; m++)
        {
            ...
            for (int b = 0; b < buffer.Count; b++)
            {
                ...
                if (InputManager.Instance.inputData.motionCommands[m].TestCheck(buffer[b].rawInputs[0].value, buffer[b].rawInputs[1].value))
...
public class InputBufferFrame
{
    ...
    public void Update()
    {
        ...
        if(rawInputs.Count == 0 || rawInputs.Count != InputManager.Instance.inputData.rawInputs.Count) { InitializeFrame(); }
...
public class InputBufferFrameState
{
    ...
    public void ResolveCommand()
    {
        ...
        switch (InputManager.Instance.inputData.rawInputs[rawInput].inputType)
        {
            case RawInput.InputType.BUTTON:
                ...
                break;
            // Here we assume we have an X and Y in InputData
            case RawInput.InputType.AXIS:
                ...
                if (Mathf.Abs(val) > DataManager.Instance.deadZone)
                {
                    HoldUp(val);
                }
                else
    ...
    {
        if(Mathf.Abs(value) < DataManager.Instance.deadZone) { return true; }
        return false;
    }
}
...
public class MotionCommand
{
    ...
    public MotionCommandDirection GetNumPadDirection(float _x, float _y)
    {
        ...
        if (Mathf.Abs(_x) > DataManager.Instance.deadZone || Mathf.Abs(_y) > DataManager.Instance.deadZone)
        {
